// fetch-feeds.js — runs in GitHub Actions (Node 20+, global fetch, zero deps).
// Pulls each RSS/Atom feed server-side and writes feeds.json next to index.html.
// On a per-source fetch failure it keeps the previously stored items, so a
// transient outage never blanks a column.

import { readFileSync, writeFileSync, existsSync } from "node:fs";

// Keep names IDENTICAL to the SOURCES names in index.html.
const FEEDS = [
...
function decode(s) {
  ...
  return s
    ...
    .replace(/<[^>]+>/g, "")            // strip stray tags inside titles
    .replace(/&amp;/g, "&")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    .replace(/&quot;/g, '"')
...
function linkOf(block) {
  ...
  // Atom: <link href="url" .../>  (prefer rel="alternate" if present)
